...
// Registration option links — replace with real payment URLs
const REGISTRATION_LINKS = {
  'ieee-srm': 'https://unstop.com/p/hize-2026-ieee-members-srmist-students-hize-2026-high-impact-zonal-events-ieee-computer-society-student-branch-srmist-1610165',
  'nonieee-srm': 'https://unstop.com/conferences/hize-2026-srm-institute-of-science-and-technology-kattankulathur-chennai-1610215',
  'ieee-nonsrm': 'https://unstop.com/p/hize-2026-srmist-students-hize-2026-high-impact-zonal-events-ieee-computer-society-student-branch-srmist-1610224',
  'nonieee-nonsrm': 'https://unstop.com/p/hize-2026-hize-2026-high-impact-zonal-events-ieee-computer-society-student-branch-srmist-1610226',
}
...
export default function RegisterForm() {
  ...
  const handleRegisterClick = () => {
    // Validate required fields before showing payment options
    if (!formData.name || !formData.regNo || !formData.email) {
      alert('Please fill in Name, Registration Number and Email before proceeding to payment.');
      return;
    }
          ...
          <div className="space-y-5">
            ...
            <div className="flex justify-end gap-3 pt-4">
              ...
              {isEditable && (
                <>
                  <button
                    onClick={isEditMode ? handleSubmit : handleRegisterClick}
                  ...
                  {showOptions && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center">
                      ...
                      <div className="relative w-full max-w-md bg-gradient-to-br from-black/95 to-orange-900/40 backdrop-blur-md rounded-2xl p-6 border border-orange-800">
                        <h3 className="text-lg font-semibold text-orange-300 mb-4">Choose registration type</h3>
                        <div className="grid grid-cols-1 gap-3">
                          <button
                            onClick={() => window.location.href = REGISTRATION_LINKS['ieee-srm']}
                            className="w-full text-left px-4 py-3 bg-black/60 border border-orange-700 rounded-lg text-orange-100 font-medium hover:bg-black/70"
                          >
                            IEEE Member + SRM — Rs399
                          </button>
                          <button
                            onClick={() => window.location.href = REGISTRATION_LINKS['nonieee-srm']}
                            className="w-full text-left px-4 py-3 bg-black/60 border border-orange-700 rounded-lg text-orange-100 font-medium hover:bg-black/70"
                          >
                            Non IEEE + SRM — Rs499
                          </button>
                          <button
                            onClick={() => window.location.href = REGISTRATION_LINKS['ieee-nonsrm']}
                            className="w-full text-left px-4 py-3 bg-black/60 border border-orange-700 rounded-lg text-orange-100 font-medium hover:bg-black/70"
                          >
                            IEEE + Non SRM — Rs799
                          </button>
                          <button
                            onClick={() => window.location.href = REGISTRATION_LINKS['nonieee-nonsrm']}
                            className="w-full text-left px-4 py-3 bg-black/60 border border-orange-700 rounded-lg text-orange-100 font-medium hover:bg-black/70"
                          >
                            Non IEEE + Non SRM — Rs899
                          </button>
                        </div>
                        <div className="flex justify-end mt-4">
                          <button onClick={() => setShowOptions(false)} className="px-4 py-2 bg-gray-800 rounded-lg text-white">Cancel</button>
                        </div>
                      </div>
                    </div>
                  )}
